...
class Utilities {
    ...
    onConnect(TOPIC) {
        ...
        scratchMqttClient.subscribe(TOPIC);
    }
    ...
    mqttCreate({SERVER,PORT,TOPIC}) {
            scratchMqttClient = new Paho.MQTT.Client(SERVER, Number(PORT), "scratchExtension" + new Date().getTime());

            scratchMqttClient.onConnectionLost = onConnectionLost;
            scratchMqttClient.onMessageArrived = onMessageArrived;

            // connect the client and THEN subscribe
            scratchMqttClient.connect({onSuccess: onConnect(TOPIC)});
    }

    mqttSend({MESSAGE}) {

        if (scratchMqttClient) {
