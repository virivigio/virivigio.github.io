...
class Utilities {
    ...
    onConnect(TOPIC) {
        ...
        window.scratchMqttClient.subscribe(TOPIC);
    }
    ...
    mqttCreate({SERVER,PORT,TOPIC}) {
            window.scratchMqttClient = new Paho.MQTT.Client(SERVER, Number(PORT), "scratchExtension" + new Date().getTime());

            window.scratchMqttClient.onConnectionLost = onConnectionLost;
            window.scratchMqttClient.onMessageArrived = onMessageArrived;

            // connect the client and THEN subscribe
            window.scratchMqttClient.connect({onSuccess: onConnect(TOPIC)});
    }

    mqttSend({MESSAGE}) {

        if (window.scratchMqttClient) {
            let mqttMessage = new Paho.MQTT.Message(MESSAGE);
            scratchMqttClient.send(mqttMessage);
        }
        else
